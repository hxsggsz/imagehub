export function imageDefault(number: number): string {
  switch (number) {
    case 1:
      return 'https://i.pinimg.com/564x/d0/b1/d2/d0b1d2467d29fdaec3b3fd1f6c627dd5.jpg'
      break
    case 2:
      return 'https://i.pinimg.com/564x/1d/80/67/1d80670e4a58c23ff39770e48ba8eac4.jpg'
      break
    case 3:
      return 'https://i.pinimg.com/564x/74/e1/70/74e170b0ba6330ec7ad0c9e688ba17d1.jpg'
      break
    case 4:
      return 'https://i.pinimg.com/564x/2e/8b/01/2e8b013bd630c641a88951f8182e27a6.jpg'
      break
    case 5:
      return 'https://i.pinimg.com/564x/cf/23/50/cf2350c395bcc2a7efc360ff180592ef.jpg'
      break
    case 6:
      return 'https://i.pinimg.com/564x/46/ef/78/46ef78f1e6b2eea3d41f1d53f54c471d.jpg'
      break

    default:
      return 'https://i.pinimg.com/564x/74/e1/70/74e170b0ba6330ec7ad0c9e688ba17d1.jpg'
      break
  }
}
